...
async def add_movie(movie_create: MovieCreate, db: AsyncSession = Depends(get_async_session)):
    return await movie.create_movie(db, movie_create)

@router.get("/", response_model=List[MovieRead])
async def list_movies(db: AsyncSession = Depends(get_async_session)):
...
async def update_movie(movie_id: int, movie_update: MovieUpdate, db: AsyncSession = Depends(get_async_session)):
    return await movie.update_movie(db, movie_id, movie_update)

@router.patch("/{movie_id}", response_model=MovieRead)
async def partial_update_movie(movie_id: int, movie_update: MovieUpdate, db: AsyncSession = Depends(get_async_session)):
