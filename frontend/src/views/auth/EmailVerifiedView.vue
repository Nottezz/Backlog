    ...
    <div class="text-center py-4">
      ...
      <template v-if="status === 'loading'">
        <div class="w-16 h-16 bg-ink-100 rounded-full flex items-center justify-center mx-auto mb-6">
          <svg class="w-8 h-8 text-ink-400 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
          </svg>
        </div>
        <p class="font-body text-ink-500">Подтверждаем адрес…</p>
      </template>
      ...
      <template v-else-if="status === 'success'">
        ...
        <h1 class="font-display text-2xl font-bold text-ink-900 mb-3">Email подтверждён!</h1>
      ...
      <template v-else-if="status === 'already-verified'">
        ...
        <h1 class="font-display text-2xl font-bold text-ink-900 mb-3">Уже подтверждён</h1>
        <p class="font-body text-ink-500 text-sm mb-8">
      ...
      <!-- No token -->
      <template v-else>
        <p class="font-body text-ink-500 mb-4">Токен подтверждения не найден</p>
